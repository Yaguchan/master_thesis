...
# python 3.1/summary_histgram_z_ba.py
...
def main():
    ...
    for T, ax in zip(Ts, axes.flat):
        pos = np.arange(len(data2[T].keys()))
        ax.bar(list(pos), data2[T].values(), width=1.0, edgecolor='black')
        ax.set_xticks(pos, data2[T].keys())
        ax.set_ylim(0, 1)
        ax.set_xlabel(r'間隔 $T\times\alpha$ [ms]')
        ax.set_ylabel('観察割合 $P$')
        ax.set_title(f'T={T}ms')
    fig.delaxes(axes.flat[-1])
    plt.tight_layout()
    plt.savefig(os.path.join(IMGDIR, f'histgram_z.png'))
    plt.show()
