    ...
    for sl, (sl_label, colors) in enumerate(zip(sl_labels, list_colors)):
        ...
        for i, (max_deltas, signal_label) in enumerate(zip(list_max_deltas, signal_labels)):
            ...
            for wavlen, color in zip(wavlens, colors):
                ...
                plt.plot(Ps, dls, marker=marker, color=color, linestyle=linestyle, label=f'{signal_label}/{sl_label}/{wavlen}ms')
        plt.yscale('log')
        plt.xscale('log')
        plt.xlim(10, 1000)
        plt.ylim(40, 300)
        plt.xlabel('基準値 $\it{T}$', fontsize=15)
        plt.ylabel('弁別閾 $\Delta\it{T}_{DL}$', fontsize=15)
        plt.tick_params(axis='both', labelsize=13)
        plt.legend(fontsize=10, title="信号$\it{S}$/間隔の長短/信号長$\it{D}$", loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(IMGDIR, f'dl_all_{sl}.png'))
        plt.show()
