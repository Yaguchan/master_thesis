...
def tone():
    ...
    for sl, (ax, sl_label, colors) in enumerate(zip(axes, sl_labels, list_colors)):
        for i, (max_deltas, signal_label) in enumerate(zip(list_max_deltas, signal_labels)):
            if i == 0: 
                linestyle = '--'
                marker = 'x'
            else: 
            ...
            for wavlen, color in zip(wavlens, colors):
                ...
                ax.plot(Ps, dls, marker=marker, color=color, linestyle=linestyle, label=f'{signal_label}/{sl_label}/{wavlen}ms')
        ax.set_yscale('log')
        ax.set_xscale('log')
        ax.set_xlim(10, 1000)
        ax.set_ylim(40, 300)
        ax.set_xlabel('基準値 $\it{T}$', fontsize=15)
        ax.set_ylabel('弁別閾 $\Delta\it{T}_{DL}$', fontsize=15)
        ax.tick_params(axis='both', labelsize=13)
        ax.legend(fontsize=10, title="信号$\it{S}$/間隔の長短/信号長$\it{D}$", loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(IMGDIR, f'dl_all2.png'))
    plt.show()
