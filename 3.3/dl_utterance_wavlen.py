...
def tone():
    ...
    # list_colors = [['lightblue', 'blue', 'darkblue'], ['lightsalmon', 'red', 'darkred']]
    list_colors = [['#9999FF', '#3333FF'], ['#FF9999', '#FF3333']]
    # list_colors = [['r', 'b'], ['r', 'b']]
    ...
    for sl, colors in enumerate(list_colors):
        for wavlen, color in zip(wavlens, colors):
            ...
            if sl == 0: 
                ...
                label_name = '$\it{T}+\Delta\it{T}$'
                marker = 'o'
            plt.plot(Ps, dls, marker=marker, color=color, label=f'{label_name}/{wavlen}ms')
    plt.yscale('log')
    plt.xscale('log')
    plt.xlim(10, 1000)
    plt.ylim(40, 400)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(IMGDIR, f'dl_utterance_wavlen.png'))
    plt.show()
